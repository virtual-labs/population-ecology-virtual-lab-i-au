  <!--
  *Developed by Amrita CREATE (Center for Research in Advanced Technologies for Education),
  *VALUE (Virtual Amrita Laboratories Universalizing Education)
  *Amrita University, India
  *http://www.amrita.edu/create
  *author:Anitha;
  *Date of modified: 13-05-2015
  -->

  <link  href="<?php getSimCss('simstyle.css'); ?>" rel="stylesheet" type="text/css" />
  <link  href="<?php getSimCss('style.css'); ?>" rel="stylesheet" type="text/css" />
  <script src="<?php getSimPath(); ?>../../../html5/js/jquery.flot.js" language="javascript"></script>
  <script src="<?php getSimPath(); ?>../../../html5/js/jquery-ui-1.8.17.custom.min.js" language="javascript"></script>
  <script src="<?php getSimPath(); ?>js/canvasjs.min.js" language="javascript"></script>
  
